...
class BlurCard(QFrame):
    def __init__(self, spec: CardSpec, *, on_click: Callable[[str], None]) -> None:
        ...
        self._release_anim = QPropertyAnimation(self, b"geometry", self)
        self._release_anim.setDuration(140)
        self._release_anim.setEasingCurve(QEasingCurve.Type.OutCubic)
    ...
    def refresh_blur(self) -> None:
        ...
        card_rect_in_window = QRect(top_left.x(), top_left.y(), self.width(), self.height())

        crop = self._window_bg.copy(card_rect_in_window)
        if crop.isNull():
            self.bg.clear()
            return
        ...
        small = crop.scaled(w, h, Qt.AspectRatioMode.IgnoreAspectRatio, Qt.TransformationMode.SmoothTransformation)
        blurred = small.scaled(crop.size(), Qt.AspectRatioMode.IgnoreAspectRatio, Qt.TransformationMode.SmoothTransformation)
    ...
    def mousePressEvent(self, event):
        ...
        g = self._base_geom
        shrink = 6
        target = QRect(g.x() + shrink, g.y() + shrink, g.width() - shrink * 2, g.height() - shrink * 2)

        self._press_anim.stop()
        self._press_anim.setStartValue(g)
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._base_geom is not None:
            ...
            self._release_anim.setStartValue(cur)
            self._release_anim.setEndValue(g)
            self._release_anim.start()

        if self.rect().contains(event.position().toPoint()):
            QTimer.singleShot(80, lambda: self.on_click(self.spec.route))
...
class HomePage(QWidget):
    def __init__(self, *, on_navigate: Callable[[str], None]) -> None:
        ...
        self._timer.timeout.connect(self._refresh_all_card_blurs)
        self._timer.start()
    ...
    def _setup_background_optional(self) -> None:
        ...
        if bg.suffix.lower() == ".gif":
            ...
            self._bg_movie.setCacheMode(QMovie.CacheMode.CacheAll)
    ...
    def _current_window_bg_pixmap(self) -> Optional[QPixmap]:
        if self._bg_movie is not None:
            ...
            if fr.isNull():
                return None
            return fr.scaled(self.size(), Qt.AspectRatioMode.IgnoreAspectRatio, Qt.TransformationMode.SmoothTransformation)
