...
class _LoadPixmapTask(QRunnable):
    ...
    def run(self) -> None:
        try:
            ...
            self.img_cache[str(self.path)] = scaled
...
class GalleryModel(QAbstractListModel):
    ...
    def __init__(self, *, dataset_dir: Path) -> None:
        ...
        self._pm_cache["__placeholder__"] = self._make_placeholder(self._icon_size)
    ...
    def data(self, index: QModelIndex, role: int = int(Qt.ItemDataRole.DisplayRole)):
        ...
        if role == int(Qt.ItemDataRole.DecorationRole):
            pm = self._pm_cache.get(spath)
            if pm is not None:
                return pm

            if spath not in self._pm_cache:
                self._pm_cache[spath] = self._pm_cache["__placeholder__"]
            ...
            return self._pm_cache["__placeholder__"]

        if role == int(Qt.ItemDataRole.DisplayRole):
            if item.split == "train":
                return f"T {item.digit} {item.index:05d}"
            return f"E {item.index:05d}"
    ...
    def _on_pixmap_ready(self, path_str: str) -> None:
        img = self._img_cache.get(path_str)
        if img is not None and not img.isNull():
            self._pm_cache[path_str] = QPixmap.fromImage(img)
...
class DatasetPage(QWidget):
    def __init__(self, *, on_home: Callable[[], None]) -> None:
        ...
        self.generate_page = GeneratePage(on_back=self.show_gallery)
        self.draw_page = DrawPage(on_back=self.show_gallery)
        ...
        self.gallery_screen.action_panel.card_generate.clicked.connect(self.show_generate)
        self.gallery_screen.action_panel.card_draw.clicked.connect(self.show_draw)
    ...
    def show_draw(self) -> None:
        self.stack.setCurrentWidget(self.draw_page)
